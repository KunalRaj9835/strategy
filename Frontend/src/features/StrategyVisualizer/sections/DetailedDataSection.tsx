...
// Type definitions
// Update the import path below to the correct location of your strategyTypes file.
// For example, if the file is in src/features/StrategyVisualizer/types/strategyTypes.ts, use:
import type { StrategyLeg, InstrumentDetails, LegType } from "../types/strategyTypes";
// If the file does not exist, create it and define the required types.
...
interface DetailedDataSectionProps {
  strategyLegs: StrategyLeg[]; // Existing (now contains legType)
  currentUnderlying: string; // Existing
  getInstrumentByToken: (token: string) => InstrumentDetails | null; // MODIFIED: Was getOptionByToken
  riskFreeRate: number; // Existing
  // MODIFIED: liveOptionChainMap to liveInstrumentChainArray if directly used, or rely on underlyingSpotPrice prop
  liveInstrumentChainArray?: any[]; // Or pass underlyingSpotPrice directly if preferred for spot
  underlyingSpotPrice: number | null; // NEW: Pass live spot price for consistency
  projectedNiftyTarget: string | number; // Existing
  projectedTargetDate: string | null; // Existing
  individualIvAdjustments: Record<string, number>; // Existing (for options)
  onIndividualIvAdjustmentChange: (token: string, value: number) => void; // Existing (for options)
  globalIvOffset: number; // Existing (for options)
  onGlobalIvOffsetChange: (updater: (prevOffset: number) => number) => void; // Existing (for options)
  onResetAllIvAdjustments: () => void; // Existing (for options)
  getScenarioIV: (token: string) => number; // Existing (for options, from StrategyVisualizer)
  multiplyByLotSize: boolean; // Existing
  onMultiplyByLotSizeChange: (checked: boolean) => void; // Existing
  multiplyByNumLots: boolean; // Existing
  onMultiplyByNumLotsChange: (checked: boolean) => void; // Existing
  sdDays: number; // Existing
  multiplier?: number; // Existing (overall strategy display multiplier)
}
...
const DetailedDataSection: React.FC<DetailedDataSectionProps> = ({
  ...
  const handleLocalGlobalIvOffsetChange = useCallback(
    (increment: number): void => {
      onGlobalIvOffsetChange((prevOffset: number) => prevOffset + increment);
    },
  ...
  const greeksSourceLabel = useMemo((): string => {
    const numericProjectedTarget = parseFloat(projectedNiftyTarget.toString());
  ...
  const singleScenarioPerLegData = useMemo(
    (): ProjectedStrategyData =>
      calculateProjectedStrategyData({
        strategyLegs,
        niftyTarget: projectedNiftyTarget.toString(), // Existing
        targetDate: projectedTargetDate ?? "", // Use projected target date or fallback to empty string
    ...
    <section className="sv-detailed-data-section">
      <div className="data-column strikewise-ivs-column">
        <h4>
          ...
          <Button variant="tertiary" size="small" onClick={onResetAllIvAdjustments}>
            Reset All IVs
          </Button>
        </h4>
