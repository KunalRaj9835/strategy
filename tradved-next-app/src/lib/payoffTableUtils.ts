// src/features/StrategyVisualizer/utils/payoffTableUtils.ts
...
/**
 * Payoff table row data
 */
interface PayoffTableRow {
  targetPrice: number;
...
/**
 * Parameters for generatePayoffTableData function
 */
interface GeneratePayoffTableDataParams {
  ...
  PAYOFF_TABLE_POINTS?: number;
  PAYOFF_TABLE_INTERVAL_STEP?: number;
  underlyingSpotPriceForPercentage?: number;
...
function calculateLegTheoreticalPrice(
  leg: StrategyLeg,
  spotAtTargetDate: number, // Spot price of the underlying at the target date
  targetDateISO: string,    // The target date for projection
  riskFreeRate: number,
  getScenarioIV: GetScenarioIV,      // Function to get scenario-adjusted IV for an option leg
  getInstrumentByToken: GetInstrumentByToken // MODIFIED: Generic instrument getter
): number {
  const instrumentDetails = getInstrumentByToken(leg.token);
  if (!instrumentDetails) return parseFloat(String(leg.price)); // Fallback if no live data
  ...
  if (leg.legType === 'option') {
    ...
    if (!instrumentDetails.expiry || instrumentDetails.strike === undefined || !instrumentDetails.optionType || instrumentDetails.legTypeDb !== 'option') {
      return parseFloat(String(leg.price)); // Fallback if option details are incomplete
    }
    ...
    if (!instrumentDetails.expiry) return parseFloat(String(leg.price)); // Need future's expiry for precise calc
  ...
  return parseFloat(String(leg.price)); // Fallback for unknown legType
}
...
/**
 * Generate payoff table data for strategy visualization
...
export const generatePayoffTableData = ({
  ...
  showPercentage,
}: GeneratePayoffTableDataParams): PayoffTableRow[] => {
  ...
  if (isNaN(centerSpotForMatrix) || centerSpotForMatrix <= 0.001) { // If center spot is still invalid or zero
      ...
      if (centerSpotForMatrix <= 0.001 && selectedLegs.length > 0) {
          const firstLegPrice = parseFloat(String(selectedLegs[0].price));
