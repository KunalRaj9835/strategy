// src/features/StrategyVisualizer/utils/strategyPayoffUtils.ts
import {
  black76Price,
  black76Greeks,
  timeToExpiry,
...
export interface ProjectedLegResult extends StrategyLeg {
...
export interface GeneratePayoffChartDataParams {
  ...
  fullInstrumentChainData?: InstrumentDetails[];
  sdDays?: number;
  PAYOFF_CHART_X_AXIS_RANGE_FACTOR?: number;
}

export interface GenerateGreeksTableDataParams {
  strategyLegs: StrategyLeg[];
  niftyTarget: string;
  targetDate: string;
  getInstrumentByToken: (token: string) => InstrumentDetails | undefined;
...
const calculateLegValueAtExpiry = (
  leg: StrategyLeg, 
...
const calculateLegPnLAtExpiry = (
  leg: StrategyLeg, 
  underlyingPriceAtExpiry: number, 
  instrumentDetails: InstrumentDetails | undefined
): number => {
  const entryPricePerShare = parseFloat(String(leg.price));
...
const calculateLegTheoreticalPrice = (
  leg: StrategyLeg,
  underlyingPrice: number, // This is the target spot for projection
  targetDateISO: string,
  riskFreeRate: number,
  getScenarioIV: (token: string) => number, // Function to get IV for an option leg
  getInstrumentByToken: (token: string) => InstrumentDetails | undefined // MODIFIED: Use new generic getter
): number => {
  const instrumentDetails = getInstrumentByToken(leg.token);
  if (!instrumentDetails) return parseFloat(String(leg.price)); // Fallback if no details

  if (leg.legType === 'option') {
    if (!instrumentDetails.expiry || !instrumentDetails.strike || !instrumentDetails.optionType || instrumentDetails.legTypeDb !== 'option') {
        return parseFloat(String(leg.price)); // Fallback for invalid option data
    }
  ...
  return parseFloat(String(leg.price)); // Fallback
};
...
const getRepresentativeIVForSD = (
  selectedLegs: StrategyLeg[], // These are strategy legs from UI
  underlyingSpotPrice: number,
  getInstrumentByToken: (token: string) => InstrumentDetails | undefined, // MODIFIED
  getScenarioIV: (token: string) => number, // NEW: To get scenario-adjusted IV
  defaultIV: number = 20 // e.g. 20%
): number => {
  if (!selectedLegs || selectedLegs.length === 0 || !underlyingSpotPrice)
    return defaultIV / 100; // Return decimal

  let closestAtmOptionLeg: StrategyLeg | null = null;
  let minDiffToAtm: number = Infinity;
  ...
  if (closestAtmOptionLeg && (closestAtmOptionLeg as StrategyLeg).token) {
    const scenarioIV = getScenarioIV((closestAtmOptionLeg as StrategyLeg).token); // Get scenario-adjusted IV (decimal)
...
// Helper function to format display expiry
const formatDisplayExpiry = (expiry: string): string => {
  try {
...
export const generatePayoffChartData = ({
...
}: GeneratePayoffChartDataParams): PayoffChartData => {
  ...
  if (centerSpotForChart <= 0 && selectedLegs.length === 0) return { points: [], sdBands: null };
  ...
  const labels: number[] = []; // Spot prices for x-axis
  const pnlOnExpiryData: number[] = [];
  const pnlOnTargetDateData: number[] = [];
  const callOIData: number[] = [];
  const putOIData: number[] = [];
  ...
  let sdLevels: SDBands | null = null;
  ...
  const pointsData: PayoffChartPoint[] = labels.map((spot, index) => ({
...
export const generateGreeksTableData = ({ // This is the function previously named calculateProjectedStrategyData
...
}: GenerateGreeksTableDataParams): GreeksTableData => {
  if (
  ...
  const projectedLegsResult: ProjectedLegResult[] = strategyLegs
    .filter((leg) => leg.selected && leg.token && leg.legType)
    .map((leg): ProjectedLegResult => {
      const instrumentDetails = getInstrumentByToken(leg.token);
      if (!instrumentDetails) {
        return {
            ...leg, 
            instrumentSymbolConcise: `${leg.buySell} ${leg.lots || 1}L ${leg.instrumentSymbol || "Data N/A"}`,
            projectedValue: null, 
            projectedPnL: 0, 
            projectedGreeks: { delta: 0, gamma: 0, theta: 0, vega: 0 }, 
            entryPrice: parseFloat(String(leg.price)), 
            ltp: null,
        };
      }
      ...
      let legGreeks: Greeks = { delta: 0, gamma: 0, theta: 0, vega: 0 };
      ...
      return {
        ...
        ltp: instrumentDetails?.lastPrice !== undefined ? parseFloat(String(instrumentDetails.lastPrice)) : null,
      };
    });
