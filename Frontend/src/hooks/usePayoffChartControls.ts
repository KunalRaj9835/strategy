...
type UsePayoffChartControlsReturn = {
  displaySpotForSlider: number;
  spotSliderMin: number;
  spotSliderMax: number;
  minDateForSliderRange: string;
  maxDateForSliderRange: string;
  dateSliderValue: number;
  handleDateSliderChange: (e: React.ChangeEvent<HTMLInputElement>) => void;
  handleResetDate: () => void;
  daysToTargetDisplay: string;
};

export const usePayoffChartControls = (
  underlyingSpotPrice: string | number | null,
  liveOptionChainMap: Map<string, OptionData>,
  currentUnderlying: string,
  strategyLegs: StrategyLeg[],
  targetDate: string,
  onTargetDateChange?: (newDate: string) => void
): UsePayoffChartControlsReturn => {
  ...
  useEffect(() => {
    ...
    const parsedSpot = parseFloat(underlyingSpotPrice as string);
    if (underlyingSpotPrice !== null && !isNaN(parsedSpot)) {
      spotToUseForRange = parsedSpot;
      ...
      const derivedSpot = parseFloat(
        String(anyOption?.marketData?.spot ?? anyOption?.marketData?.futures)
      );
        ...
        if (leg.selected && leg.expiry) {
          try {
            ...
            if (!isNaN(legExpiryDate.getTime())) {
              if (!foundMaxExpiry || legExpiryDate > latestExpiryDateInStrategy) {
