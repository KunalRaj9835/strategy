// src/features/utils/optionPricingUtils.tsx
...
//   return sign*y;
// }
// export function normCDF_alternative(x: number): number {
//   if (x === Infinity) return 1;
//   if (x === -Infinity) return 0;
 ...
 * @param F - Forward/Futures price (or Spot price).
 * @param K - Strike price.
 * @param T - Time to expiry in years.
 * @param r - Risk-free interest rate (annualized decimal).
 * @param sigma - Implied volatility (annualized decimal, e.g., 0.2 for 20%).
...
/**
 * Black-76 Greeks.
 * @returns { delta, gamma, theta (daily), vega (for 1% IV change) }
 */
...
const parseExpiryString = (expiryStr: string): Date | null => {
 ...
 * @param expiryInput - Expiry date string (e.g., "30MAY2024") or Date object.
 * @param calculationDate - The date from which to calculate TTE.
 ...
 */
export function timeToExpiryDays(
  expiryInput: string | Date,
  calculationDate: Date = new Date()
): number {
    const expiryDateTime = expiryInput instanceof Date ? expiryInput : parseExpiryString(expiryInput);
 ...
 * @param optionType - "CE" for Call, "PE" for Put.
 * @returns Payoff value (intrinsic value).
 */
