...
export async function setupWebSocket(io) {
  ...
  //console.log("🔄 (Main Feed) Token Meta Map built with", Object.keys(tokenMetaMap).length, "tokens.");
...
const bankniftyTokens = scripData
  ...
  console.log(optionsAnsFuturesTokens)
  ...
  ws.connect()
    .then(() => {
      ...
      ws.on("tick", (data) => handleTick(data, tokenMetaMap, io)); // Pass local tokenMetaMap
              ws.on('error', (error) => {
          console.error("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!");
          console.error("🛑 (Option Feed) A WebSocket error occurred:", error);
          console.error("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!");
      });
    ...
    .catch((err) => {
       // --- FIX: Improved error logging to see the full error object ---
      console.error("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!");
      console.error("🔥 (Option Feed) SmartAPI initial connection FAILED. Full error below:");
      console.error(err); // Log the entire error object for more detail
      console.error("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!");
  ...
  // console.log("Current Token Meta Map:", currentTokenMetaMap); // Debugging line to check token meta map
    if (data === "pong") return;
...
function processInstrumentTick(rawTick, currentTokenMetaMap, currentTickTime) {
    try {
     ...
     // console.log("Processing tick for token:", token, "with meta:", meta, "isOption:", isOption, "isFuture:", isFuture);
      if (isOption) {
      ...
      const bestBidRaw = rawTick.bp || rawTick.best_5_buy_data?.[0]?.price;
      const bestAskRaw = rawTick.sp || rawTick.best_5_sell_data?.[0]?.price;
     //  console.log("Best Bid Raw:", bestBidRaw, "Best Ask Raw:", bestAskRaw);
      return {
